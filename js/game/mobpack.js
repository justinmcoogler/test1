...
import { IMPORTED_MODELS } from '../gfx/mobpack-imported.js';
...
export async function registerImportedMobs(renderer) {
  // assets/mobs/*.bbmodel drop-ins (MOB_MODELS) plus the licensed pack
  // (IMPORTED_MODELS, generated by tools/import-mobpack.mjs). Both share the
  // {texW,texH,rig,texture,parts} shape and register the same way. Drop-ins win
  // on an id collision (spread last).
  const ALL = { ...IMPORTED_MODELS, ...MOB_MODELS };
  // A hand-authored native owns its id. This loop runs async (after each texture
  // decodes) and does delete-then-register, so without this guard a pack mob
  // sharing an id would silently replace the remade model well after startup —
  // which is exactly what happened to `zombie`.
