...
// MC facing → our packed facing (bits 0-1 dir, bit 2 top half, bit 3 trapdoor-open).
...
function facingOf(rawId, shape) {
  ...
  if (shape === 'panel') { // trapdoor: keep hinge wall, top/bottom half, and open state
    if (fm) v |= MC_FACE[fm[1]];
    if (/half=top/.test(rawId)) v |= 4;
    if (/open=true/.test(rawId)) v |= 8;
    return v;
  }
...
for (const [raw, i] of Object.entries(palette)) {
  ...
  packed[i] = (id & 0x0fff) | ((facingOf(raw, shape) & 0xf) << 12);
...
const header = '// GENERATED by tools/bake-town.mjs — do not edit by hand.\n'
  ...
  + `// ${count.toLocaleString()} non-air cells. Uint16: bits 0-11 block id, 12-15 facing.\n`
